...
### 1.6 Terraform plan
Now that we've defined our service configuration in our `terraforn/main.tf` file we want to observe what will happen if we were to deploy and apply these changes to our Fastly account. We can do this by running the [`terraform plan`](https://www.terraform.io/docs/commands/plan.html) CLI command:
```sh
