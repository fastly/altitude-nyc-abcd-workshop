...
```sh
$ terraform plan -state=./.state ./terraform
```
> _What's this?_ 
...
#### 1.6.2 A note about state
You may have noticed in the `terraform plan` command we passed a state argument via a flag `-state`. This tells Terraform where to persist the state of the last-known arguments and attributes for all resources in our configuration. These contents known as "state" can be stored locally as a JSON file (local state) or stored in a remote shared location such as a GCS bucket (remote state) so configuration can be shared across team members and CI.
...
```sh
$ terraform apply -state=./.state -var='fastly_api_token=<YOUR_TOKEN>' ./terraform
```
> _What's this?_ 
